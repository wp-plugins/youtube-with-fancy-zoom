...
Description: Youtube with fancy zoom plugin is a media viewing application that supports webs most popular youtube video. 
...
function g_ywfz_install() 
{
	...
	if($wpdb->get_var("show tables like '". WP_G_YWFZ_TABLE . "'") != WP_G_YWFZ_TABLE) 
	{
		...
		$sSql = $sSql . "`ywfz_code`='3JQ6urEFLY8',";
		$sSql = $sSql . "`ywfz_img`='Youtube Thumbnail', ";
		$sSql = $sSql . "`ywfz_size`='0', `ywfz_imglink`='', ";
		$sSql = $sSql . "`ywfz_status`='Yes', `ywfz_sidebar`='Yes';";
		$wpdb->query($sSql);
...
{
	add_options_page('Youtube with fancy zoom', 'Youtube with fancy zoom', 'manage_options', __FILE__, 'g_ywfz_admin_option' );
	add_options_page('Youtube with fancy zoom', '', 'manage_options', "youtube-with-fancy-zoom/ywfz_widget_setting.php",'' );
}
